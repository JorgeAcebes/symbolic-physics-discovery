...
def render(cell_text, cell_colors, models, out_path):
    ...
    legend = [
        ...
        mpatches.Patch(color=COLOR_APPROX, label=r"Aproximada ($10^{-4} \leq \mathrm{MSE} < 10^{-2}$)"),
        mpatches.Patch(color=COLOR_INCORRECT, label=r"Incorrecta ($\mathrm{MSE} \geq 10^{-2}$)"),
    ]
