...
Las redes neuronales multicapa o perceptrones multicapa (MLP) ajustan sus parámetros para modelar relaciones altamente no lineales, sacrificando interpretabilidad analítica. Estructuralmente, la red procesa la información a través de capas ocultas intermedias totalmente conectadas (\textit{fully connected}), mostradas en la Figura \ref{fig:mlp}, las cuales proyectan los datos a nuevos espacios de representación mediante transformaciones afines seguidas de no linealidades para extraer características abstractas.


En el marco de este estudio, los distintos MLPs fueron implementados en \texttt{PyTorch} y entrenados por lotes (\textit{batches}) mediante descenso de gradiente estocástico (\texttt{Adam}).
\FloatBarrier
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}[
...
\begin{equation}
y \approx \Theta(X) \boldsymbol{\xi},\end{equation}
...
Tras el ajuste, la expresión analítica emerge como la superposición de los términos activos ($\mathcal{S}$):\begin{equation}
y \approx \sum_{i \in \mathcal{S}} \theta_i(X) \xi_i.
\end{equation}
...
En el marco de este estudio, el espacio de funciones se genera combinando las variables mediante operadores binarios ($\{+, -, \times, \div, \wedge\}$) y transformaciones unarias ($\{x^{-1}, x^2, e^x, \cos x, \sin x, \ln x\}$). Para garantizar la extracción de relaciones algebraicas interpretables, se impone una restricción explícita de complejidad que limita el número de nodos permitidos en los árboles de expresión. 
